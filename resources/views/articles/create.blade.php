    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                <div id="autoSaveStatus" class="text-sm text-gray-600"></div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded">
                <p class="text-sm">
                    <strong>Auto-save:</strong> Your work is automatically saved every 60 seconds to prevent data loss. Press <kbd class="px-2 py-1 bg-blue-100 rounded">Ctrl+S</kbd> to save manually.
                </p>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('articles.store') }}" enctype="multipart/form-data" id="articleForm" target="_blank">
                        @csrf
                            ...
                            <div class="lg:col-span-2 space-y-6">
                                ...
                                <div>
                                    ...
                                    <textarea name="summary" id="summary" rows="3"
                                        ...
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('summary') }}</textarea>
                                </div>
                                ...
                                @include('articles.partials.draft-reason-options', ['selectedKey' => old('draft_reason_preset', '')])
                                ...
                                <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 sticky top-6">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div id="infoFields" class="space-y-2">
                                            @if(old('info'))
                                                @foreach(old('info') as $index => $info)
    ...
    <script type="module">
                ...
                document.addEventListener('keydown', function(e) {
                    if ((e.ctrlKey || e.metaKey) && e.key === 's') {
                        ...
                        if (statusEl) {
                            ...
                            statusEl.innerHTML = '<span class="text-green-600 font-semibold">✓ Manually saved!</span>';
                            setTimeout(() => {
        ...
        function autoSaveFormData() {
            ...
            try {
                ...
                localStorage.setItem('draftsencyclopedia_article_draft', JSON.stringify(formData));

                const statusEl = document.getElementById('autoSaveStatus');
                if (statusEl) {
                    statusEl.innerHTML = '<span class="text-green-600">✓ Saved ' + new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) + '</span>';
                }
            } catch (e) {
                console.error('Error saving draft:', e);
                const statusEl = document.getElementById('autoSaveStatus');
                if (statusEl) {
                    statusEl.innerHTML = '<span class="text-red-600">⚠ Save failed</span>';
                }
            }
        }
        ...
        function restoreSavedData() {
            try {
                const saved = localStorage.getItem('draftsencyclopedia_article_draft');
                if (!saved) return;
                ...
                if (hoursSince > 168) {
                    localStorage.removeItem('draftsencyclopedia_article_draft');
                    return;
                }

                if (document.getElementById('title')?.value) return;

                if (confirm('Found unsaved work from ' + new Date(data.timestamp).toLocaleString() + '. Restore it?')) {
                    if (data.title) document.getElementById('title').value = data.title;
                    if (data.content && editor) editor.setData(data.content);
                    if (data.summary) document.getElementById('summary').value = data.summary;
                    if (data.status) document.getElementById('status').value = data.status;
                }
            } catch (e) {
                console.error('Error restoring draft:', e);
                localStorage.removeItem('wikiengine_article_draft');
            }
        }
